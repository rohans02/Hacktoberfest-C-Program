...
int main()
{
    ...
    scanf("%d%d-",&m,&n);
    //Checking the condition for the possibility of matrix multiplication
    if(c==m)
    {
        ...
        printf("\n\n");
        for(i=0;i<r;++i)                                  //Multiplying matrix A and matrix B
        {
            for(j=0;j<n;++j)
            {
                sum[i][j]=0;
    ...
    {
        printf("!!!!Matrix multiplication is not possible please re-enter the rows and columns!!!!");
        goto label;
    }
    return(0);
